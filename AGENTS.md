...
lessons/                        ← runnable lesson scripts - they start with s01e01, s01e02, etc. (season 1, episode 1, etc.) 
```
...
- Favour `pathlib.Path` over `os.path`.
...
- Ruff: `uv run ruff check src/ examples/`
...
- use `uv add <module>` to install new modules from pypi
- use `uv run <file>` to run a file instead of `python <file>`

example:
- to run lesson 1 code do `uv run python -m src.lessons.lesson1`